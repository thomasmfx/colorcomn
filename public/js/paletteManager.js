...
const colorList = document.querySelector('.palette-colors');
...
function createColorElement(id, name, code) {
  ...
  colorItem.className = 'palette-colors__item';

  const colorBtn = document.createElement('button');
  colorBtn.type = 'button';
  colorBtn.className = 'palette-colors__btn-color';
  colorBtn.dataset.id = id;
  colorBtn.setAttribute('aria-label', `Remove color: ${name}/${code}`);
  colorBtn.style.backgroundColor = code;

  const removeColorIcon = document.createElement('i');
  removeColorIcon.className = 'bx bx-x-circle palette-colors__btn-color-icon';
  removeColorIcon.ariaHidden = 'true';
  if (getTextColorFromHex(code) === 'white') {
    removeColorIcon.classList.add('palette-colors__btn-color-icon--light');
  }

  colorBtn.appendChild(removeColorIcon);
  colorItem.appendChild(colorBtn);

  colorItem.addEventListener('click', () => removeColor(id));
